...
async function main() {
  ...
  try {
    const headline = await qwenClient.generateHeadline('Bitcoin mencapai harga tertinggi baru di $68,000');
  ...
  try {
    ...
    console.log('  Estimated cost: $' + qwenClient.calculateCost(article.usage.totalTokens).toFixed(4));
  } catch (error: any) {
    console.error('❌ Error:', error.message);
  }
  console.log('\n');
  ...
  try {
    const response = await qwenClient.generate({
      messages: [
        {
          role: 'system',
          content: 'Anda adalah analis kripto. Berikan analisis singkat dan jelas.',
        },
        {
          role: 'user',
          content: 'Analisis pergerakan harga Ethereum minggu ini dalam 3 poin utama.',
        },
      ],
      temperature: 0.7,
      maxTokens: 6000,
    ...
    console.log('💰 Cost: $' + qwenClient.calculateCost(response.usage.totalTokens).toFixed(4));
    
    if (response.reasoningContent) {
      console.log('\n🧠 Model had reasoning (length):', response.reasoningContent.length, 'chars');
    }
  } catch (error: any) {
    console.error('❌ Error:', error.message);
  }
