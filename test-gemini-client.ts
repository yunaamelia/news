...
import { geminiFlash, geminiPro } from './app/lib/ai/gemini-client';

async function main() {
  console.log('🧪 Testing Gemini Client (FREE AI Solution)\n');
  console.log('📊 Model Info:');
  console.log(geminiFlash.getModelInfo());
  console.log('\n' + '='.repeat(70) + '\n');
  ...
  console.log('📝 Test 2: Generate BBCA stock article');
  console.log('─'.repeat(70));
  try {
    const article = await geminiFlash.generateArticle({
      ...
      category: 'SAHAM',
      targetWords: 150,
      includeData: 'Volume perdagangan: 45,2 juta lembar. Market cap: Rp 1.200 triliun.',
    });
  ...
  console.log('📝 Test 3: Generate 5 headline variations');
  console.log('─'.repeat(70));
  try {
  ...
  console.log('📝 Test 4: Batch generate 3 articles at once');
  console.log('─'.repeat(70));
  try {
    const topics = [
      'Bitcoin naik 5%',
  ...
  const geminiCost = geminiFlash.calculateCost(totalPromptTokens, totalCompletionTokens);
  const gpt4Cost = (totalPromptTokens * 10 / 1_000_000) + (totalCompletionTokens * 30 / 1_000_000);
  const claudeCost = (totalPromptTokens * 3 / 1_000_000) + (totalCompletionTokens * 15 / 1_000_000);

  console.log(`Articles: ${articlesPerMonth} per month`);
  console.log(`\n💰 Cost Comparison:`);
  console.log(`  Gemini Flash:  $${geminiCost.toFixed(2)}/month ✅ TERMURAH!`);
  console.log(`  GPT-4 Turbo:   $${gpt4Cost.toFixed(2)}/month`);
  console.log(`  Claude Sonnet: $${claudeCost.toFixed(2)}/month`);
  console.log(`\n🎉 Hemat: ${((1 - geminiCost / gpt4Cost) * 100).toFixed(1)}% vs GPT-4!`);
